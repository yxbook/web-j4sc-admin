...
<div>
    <Table highlight-row :ref="refs" :columns="columnsList" :data="thisTableData" @on-current-change="handleSelect" border disabled-hover></Table>
</div>
        ...
        on: {
            'click': () => {
                if (!currentRow.editting) {
                    ...
                    vm.thisTableData = JSON.parse(JSON.stringify(vm.edittingStore));
                    putRole(vm.thisTableData[index]).then(data => {
                        if (data.status === 200) {
